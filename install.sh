...
build_fleet_config() {
    ...
    prompt_var "SSH user on the primary server"     ssh_user "<your-linux-user>"
    prompt_var "SSH port on the primary server"      ssh_port "22"
    prompt_var "API base URL (https://...)"          api_url  "https://api.tunnels.example.com"
    prompt_var "Base domain for tunnel hostnames"    base_domain "tunnels.example.com"
