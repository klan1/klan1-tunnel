...
#   deploy/Caddyfile.tunels    → se concatena al /etc/caddy/Caddyfile
...
#   scp ~/Develop/hermes/klan1-tunnel/deploy/Caddyfile.tunels      ai1:/tmp/
...
cp /etc/caddy/Caddyfile /etc/caddy/Caddyfile.bak.pre-tunels.$(date +%Y%m%d-%H%M%S)

# Concatenar el bloque nuevo (ya está en /tmp/Caddyfile.tunels.new)
cat /tmp/Caddyfile.tunels.new >> /etc/caddy/Caddyfile

# Verificar el Caddyfile final (debe tener vscode + opencode + 10 tunels)
